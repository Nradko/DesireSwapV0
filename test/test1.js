...
function token0ToLiquidity(amountToAdd, currentPrice, lowestIndexToAdd, highestIndexToAdd, multiplier) {
  ...
  if (s1 <= sc) return token0ToAdd.mul((s0 * s1) / (s1 - s0));
  else if (s0 < sc && sc < s1) return token0ToAdd.mul((s0 * sc) / (sc - s0));
  else sc <= s0;
  ...
  it('TESTing...', async function () {
    ...
    for (let step = 0; step < 7; step++) {
      ...
      await router.connect(A2).exactInputSingle({
        ...
        deadline: '1000000000000000000000000',
        amountIn: BigNumber.from('10000000000000000000000000')
          .mul(BigNumber.from(((step + 1) * (step + 2)) / 2))
          .toString(),
      ...
      await router.connect(A2).exactInputSingle({
        ...
        recipient: A2.address,
        deadline: '1000000000000000000000000',
        amountIn: BigNumber.from('20000000000000000000000000')
          .mul(((step + 1) * (step + 2)) / 2)
          .toString(),
