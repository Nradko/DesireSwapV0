...
contract PositionViewer {
    ...
    function getCurrentPrice(address poolAddress)
    public view
    returns(uint256){
    ...
    external view
    returns (PositionData[] memory positionDataList, uint256 currentPrice){
        IDesireSwapV0Pool pool = IDesireSwapV0Pool(poolAddress);
        ...
        currentPrice = getCurrentPrice(poolAddress);
    }
}