...
contract Ticket is ITicket {
  ...
  function getTicketOwner(uint256 ticketId) external view override returns (address) {
    return _owners[ticketId];
  }

  function getTicketData(uint256 ticketId) external view override returns (TicketData memory) {
