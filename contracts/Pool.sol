...
contract DesireSwapV0Pool is Ticket, IDesireSwapV0Pool {
  ...
  uint256 public protocolFeePart = (2 * D) / 10;
  ...
  int24 public inUseRange;
  int24 public highestActivatedRange;
  int24 public lowestActivatedRange;
  ...
  function activate(int24 index) public {
    require(!ranges[index].activated, 'DesireSwapV0: POSITION_ALREADY_ACTIVATED');
    if (index > highestActivatedRange) {
      if (!ranges[index - 1].activated) activatePrivate(index - 1);
      ranges[index].sqrtPriceBottom = (ranges[index - 1].sqrtPriceBottom * sqrtRangeMultiplier) / D;
  ...
  function _swapInRange(
    ...
    require((zeroForOne && amountOut <= h.value01) || (!zeroForOne && amountOut <= h.value00), 'DesireSwapV0: INSUFFICIENT_POSITION_LIQUIDITY');
    ...
    if (protocolFeeIsOn) collectedProtocolFee = (collectedFee * protocolFeePart) / D;
    ...
    if (zeroForOne) {
      ...
      require(
        ...
        'DesireSwapV0: LIQ_COEFFICIENT_IS_TOO_LOW'
      ); //assure that after swap there is more o r equal liquidity. If PoolHelper.AmountIn works correctly it can be removed.
      //!!
      _modifyRangeReserves(index, amountIn - collectedProtocolFee, amountOut, true, false);
    }
    // token1 for token0 // token1 in; token0 out;
    else {
      //??
      require(
        PoolHelper.LiqCoefficient(h.value00 - amountOut, h.value01 + amountInHelp, h.value00, h.value11) >= PoolHelper.LiqCoefficient(h.value00, h.value01, h.value10, h.value11),
        'DesireSwapV0: LIQ_COEFFICIENT_IS_TOO_LOW'
      ); //assure that after swao there is more or equal liquidity. If PoolHelper.AmountIn works correctly it can be removed.
      //!!
      _modifyRangeReserves(index, amountOut, amountIn - collectedProtocolFee, false, true);
    }
  ...
  function swap(
    ...
    //???
    if (s.zeroForOne) {
      require(h.balance0 >= h.lastBalance0 + amountRecieved && h.balance1 >= h.lastBalance1 - amountSend, 'DesireSwapV0: BALANCES_ARE_T0O_LOW');
    } else {
      require(h.balance1 >= h.lastBalance1 + amountRecieved && h.balance0 >= h.lastBalance0 - amountSend, 'DesireSwapV0: BALANCES_ARE_T0O_LOW');
    }
  ...
  function _printOnTicket0(
    ...
    if (ranges[index].supplyCoefficient != 0) {
      ...
      _ticketSupplyData[ticketId][index] = PoolHelper.LiqCoefficient(reserve0, reserve1, sqrtPriceBottom, sqrtPriceTop);
    }
  ...
  function collectFee(address token, uint256 amount) external override {
    ...
    require(IERC20(token0).balanceOf(address(this)) >= totalReserve0 && IERC20(token1).balanceOf(address(this)) >= totalReserve1);
