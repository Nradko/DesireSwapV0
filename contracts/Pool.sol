...
/// Swapping
	...
	returns( uint256 amountIn)
    {
		...
		helpData memory h = helpData({
			...
			value10: ranges[index].sqrtPriceBottom, value11: 0
		});
    ...
    {       
        ...
        if(s.amount < 0){
            ...
            while( remained > usingReserve && 
				(s.zeroForOne ? 
					...
					: sqrtPriceLimit < ranges[usingRange].sqrtPriceBottom
				|| sqrtPriceLimit == 0)
			){
		...
		if( s.zeroForOne){
			require( h.balance0 >= h.lastBalance0 + amountRecieved && h.balance1 >= h.lastBalance1 - amountSend,
	            'DSV0POOL(swap): BALANCES_ARE_T0O_LOW');
        } else {
...
///
	//initialize helper
	function startingSqrtPriceBottom(int24 _startingInUseRange)
	private view
	returns(uint256 startingSqrtPriceBottom_)
	{
	...
	function initialize(int24 _startingInUseRange)
	external override 
	{
